...
// 【編集規約・禁止事項】(2026-08-06 制定)
// 1. 医療機関・医療サービスへのアフィリエイトリンクは一切貼らない。
//    医療広告ガイドラインはアフィリエイターも規制対象で、治療の体験談・
//    ビフォーアフター等の表現が禁止されるため、事故リスクに見合わない。
//    (オンライン診療・禁煙外来・美容医療・自由診療クリニック等すべて対象)
// 2. 新ジャンル追加は「ASP案件が実在する × 個人が上位を取れる × 内部リンクが自然」
//    の3条件を満たすもののみ。案件があることは参入理由にならない。
// 3. 金融商品(証券口座・保険・カード)は、個別商品の推奨をせず制度解説に留める。
