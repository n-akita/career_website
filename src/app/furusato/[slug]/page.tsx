...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const article = getArticle("furusato", slug);
  if (!article) return {};
  const ogParams = new URLSearchParams({ title: article.title, category: "furusato" });
  const imageUrl = `https://nara-career.com/api/og/article?${ogParams}`;
  return {
    title: article.title,
    description: article.description,
    alternates: {
      canonical: `https://nara-career.com/furusato/${slug}`,
    },
    openGraph: {
      type: "article",
      title: article.title,
