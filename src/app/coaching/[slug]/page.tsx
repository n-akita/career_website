...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const article = getArticle("coaching", slug);
  if (!article) return {};
  const ogParams = new URLSearchParams({ title: article.title, category: "coaching" });
  const imageUrl = `https://nara-career.com/api/og/article?${ogParams}`;
  return {
    title: article.title,
    description: article.description,
    alternates: {
      canonical: `https://nara-career.com/coaching/${slug}`,
    },
    openGraph: {
      type: "article",
      title: article.title,
