...
const PICK_SLUGS: { category: string; slug: string }[] = [
  { category: "mindset", slug: "dx-talent-salary" },
  { category: "tenshoku", slug: "jtc-regret-checklist" },
  { category: "mindset", slug: "venture-vs-enterprise-reality" },
  { category: "tenshoku", slug: "job-change-count" },
  { category: "sidejob", slug: "how-to-start-sidejob" },
  { category: "mindset", slug: "two-billion-yen-night" },
];

export default function CareerTop() {
  // キャリアジャンル配下のみ（/taishoku 等の独立ジャンルは含めない）
  const articles = CAREER_CATEGORIES.flatMap((cat) => getArticlesByCategory(cat))
    ...
    <>
                ...
                <li key={`${a.category}/${a.slug}`}>
                  ...
                  >
                    <span className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded mt-1 shrink-0 ${
                      ...
                      {a.category === "mindset" ? "career" : a.category}
                    </span>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 py-20 text-center text-white">
          ...
          <h2 className="text-2xl md:text-4xl font-bold mb-4">まず読んでほしい2本</h2>
          <p className="text-blue-200 mb-8 max-w-lg mx-auto leading-relaxed">
            年収400万から3.5倍までの全記録と、ベンチャーから大手へ移るための完全ガイド。
            このサイトの核になる2本です。
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            <Link
              href="/career/mindset/career-story"
              className="group inline-flex items-center gap-2 bg-white text-blue-700 font-bold px-8 py-4 rounded-xl text-base hover:bg-blue-50 transition-all hover:shadow-xl hover:shadow-black/10"
            >
              転職全記録を読む
            ...
            <Link
              ...
              className="group inline-flex items-center gap-2 border border-white/40 text-white font-bold px-8 py-4 rounded-xl text-base hover:bg-white/10 transition-all"
            >
              完全ガイドを読む
              <svg className="w-5 h-5 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" />
